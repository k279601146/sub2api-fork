...
type SystemSettings struct {
	...
	EnableFingerprintUnification       bool // 是否统一 OAuth 账号的指纹头（默�?true�?
	EnableMetadataPassthrough          bool // 是否透传客户端原�?metadata（默�?false�?
	EnableCCHSigning                   bool // 是否�?billing header cch 进行签名（默�?false�?
	EnableAnthropicCacheTTL1hInjection bool // 是否�?Anthropic OAuth/SetupToken 请求体注�?1h cache_control ttl（默�?false�?
...
// StreamTimeoutAction 流超时处理方式常�?
const (
	StreamTimeoutActionTempUnsched = "temp_unsched" // 临时不可调度
	StreamTimeoutActionError       = "error"        // 标记为错误状�?
	StreamTimeoutActionNone        = "none"         // 不处�?
)
...
// RectifierSettings 请求整流器配�?
type RectifierSettings struct {
	Enabled                  bool     `json:"enabled"`                    // 总开�?
	...
	APIKeySignatureEnabled   bool     `json:"apikey_signature_enabled"`   // API Key 签名整流开�?
...
// Beta Policy 策略常量
const (
	BetaPolicyActionPass   = "pass"    // 透传，不做任何处�?
	BetaPolicyActionFilter = "filter"  // 过滤，从 beta header 中移除该 token
	BetaPolicyActionBlock  = "block"   // 拦截，直接返回错�?
	BetaPolicyScopeAll     = "all"     // 所有账号类�?
	BetaPolicyScopeOAuth   = "oauth"   // �?OAuth 账号
...
type BetaPolicyRule struct {
	BetaToken            string   `json:"beta_token"`                       // beta token �?
...
// DefaultOverloadCooldownSettings 返回默认的过载冷却配置（启用�?0分钟�?
...
// OpenAI �?"fast 模式" 通过请求体中�?service_tier 字段识别�?
//   - "priority"（客户端可传 "fast"，归一化为 "priority"）：fast 模式
//   - "flex"：低优先级模�?
//   - 省略：normal 默认
//
// 本策略复�?BetaPolicyAction*/BetaPolicyScope* 常量语义，只是匹配键�?
// anthropic-beta header 换成 body �?service_tier 字段�?
const (
	OpenAIFastTierAny      = "all"      // 匹配任意已识别的 service_tier
	OpenAIFastTierPriority = "priority" // 仅匹�?fast（priority�?
	OpenAIFastTierFlex     = "flex"     // 仅匹�?flex
)
...
// DefaultOpenAIFastPolicySettings 返回默认�?OpenAI fast 策略配置�?
// 默认对所有模型的 priority（fast）请求执�?filter，即剔除 service_tier 字段�?
// 让上游按 normal 优先级处理�?
//
// 为什�?ModelWhitelist 为空�?对所有模型生效）�?
// codex 客户端的 service_tier=fast 是用户级开关，�?model 字段正交。即�?
// 用户使用 gpt-4 + fast，priority 配额仍会被消耗。如果默认规则只�?
// gpt-5.5*�?�?gpt-4 + fast 透传 priority 上游" 这条路径就会绕过策略�?
// �?codex 真实语义对齐，默认对所有模型生效；管理员若需要只针对特定
// 模型，可�?admin UI 中显式配�?model_whitelist�?
