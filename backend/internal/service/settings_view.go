...
type SystemSettings struct {
	...
	TotpEnabled                      bool // TOTP 双因素认�?	LoginAgreementEnabled            bool
	...
	UsageWindowLimitUnits        float64
	UsageWeeklyLimitUnits        float64
	...
	// Dev2 model name (exposed via GET /internal/dev2/config)
	...
	// 分组隔离：允许未分组 Key 调度（默�?false �?403�?	AllowUngroupedKeyScheduling bool

	// Backend 模式：禁用用户注册和自助服务，仅管理员可登录
	BackendModeEnabled bool

	// Gateway forwarding behavior
	EnableFingerprintUnification       bool // 是否统一 OAuth 账号的指纹头（默�?true�?	EnableMetadataPassthrough          bool // 是否透传客户端原�?metadata（默�?false�?	EnableCCHSigning                   bool // 是否�?billing header cch 进行签名（默�?false�?	EnableAnthropicCacheTTL1hInjection bool // 是否�?Anthropic OAuth/SetupToken 请求体注�?1h cache_control ttl（默�?false�?
...
type StreamTimeoutSettings struct {
	// Enabled 是否启用流超时处�?	Enabled bool `json:"enabled"`
	// Action 超时后的处理方式: "temp_unsched" | "error" | "none"
	Action string `json:"action"`
	// TempUnschedMinutes 临时不可调度持续时间（分钟）
	TempUnschedMinutes int `json:"temp_unsched_minutes"`
	// ThresholdCount 触发阈值次数（累计多少次超时才触发�?	ThresholdCount int `json:"threshold_count"`
	// ThresholdWindowMinutes 阈值窗口时间（分钟�?	ThresholdWindowMinutes int `json:"threshold_window_minutes"`
}

// StreamTimeoutAction 流超时处理方式常�?const (
	StreamTimeoutActionTempUnsched = "temp_unsched" // 临时不可调度
	StreamTimeoutActionError       = "error"        // 标记为错误状�?	StreamTimeoutActionNone        = "none"         // 不处�?)
...
// RectifierSettings 请求整流器配�?type RectifierSettings struct {
	Enabled                  bool     `json:"enabled"`                    // 总开�?	ThinkingSignatureEnabled bool     `json:"thinking_signature_enabled"` // Thinking 签名整流
	ThinkingBudgetEnabled    bool     `json:"thinking_budget_enabled"`    // Thinking Budget 整流
	APIKeySignatureEnabled   bool     `json:"apikey_signature_enabled"`   // API Key 签名整流开�?	APIKeySignaturePatterns  []string `json:"apikey_signature_patterns"`  // API Key 自定义匹配关键词
}
...
// Beta Policy 策略常量
const (
	BetaPolicyActionPass   = "pass"   // 透传，不做任何处�?	BetaPolicyActionFilter = "filter" // 过滤，从 beta header 中移除该 token
	BetaPolicyActionBlock  = "block"  // 拦截，直接返回错�?
	BetaPolicyScopeAll     = "all"     // 所有账号类�?	BetaPolicyScopeOAuth   = "oauth"   // �?OAuth 账号
	BetaPolicyScopeAPIKey  = "apikey"  // �?API Key 账号
	BetaPolicyScopeBedrock = "bedrock" // �?AWS Bedrock 账号
)

// BetaPolicyRule 单条 Beta 策略规则
type BetaPolicyRule struct {
	BetaToken            string   `json:"beta_token"`                       // beta token �?	Action               string   `json:"action"`                           // "pass" | "filter" | "block"
	Scope                string   `json:"scope"`                            // "all" | "oauth" | "apikey" | "bedrock"
	ErrorMessage         string   `json:"error_message,omitempty"`          // 自定义错误消�?(action=block 时生�?
	ModelWhitelist       []string `json:"model_whitelist,omitempty"`        // 模型匹配模式列表（为�?对所有模型生效）
	FallbackAction       string   `json:"fallback_action,omitempty"`        // 未匹配白名单的模型的处理方式
	FallbackErrorMessage string   `json:"fallback_error_message,omitempty"` // 未匹配白名单时的自定义错误消�?(fallback_action=block 时生�?
}
...
type OverloadCooldownSettings struct {
	// Enabled 是否在收�?29时暂停账号调�?	Enabled bool `json:"enabled"`
...
type RateLimit429CooldownSettings struct {
	...
	// CooldownSeconds 默认回避时长（秒�?	CooldownSeconds int `json:"cooldown_seconds"`
}

// DefaultOverloadCooldownSettings 返回默认的过载冷却配置（启用�?0分钟�?func DefaultOverloadCooldownSettings() *OverloadCooldownSettings {
...
// DefaultRateLimit429CooldownSettings 返回默认�?29回避配置（启用，5秒）
...
// DefaultBetaPolicySettings 返回默认�?Beta 策略配置
...
// OpenAI �?"fast 模式" 通过请求体中�?service_tier 字段识别�?//   - "priority"（客户端可传 "fast"，归一化为 "priority"）：fast 模式
//   - "flex"：低优先级模�?//   - 省略：normal 默认
//
// 本策略复�?BetaPolicyAction*/BetaPolicyScope* 常量语义，只是匹配键�?// anthropic-beta header 换成 body �?service_tier 字段�?const (
	OpenAIFastTierAny      = "all"      // 匹配任意已识别的 service_tier
	OpenAIFastTierPriority = "priority" // 仅匹�?fast（priority�?	OpenAIFastTierFlex     = "flex"     // 仅匹�?flex
)
...
// DefaultOpenAIFastPolicySettings 返回默认�?OpenAI fast 策略配置�?// 默认对所有模型的 priority（fast）请求执�?filter，即剔除 service_tier 字段�?// 让上游按 normal 优先级处理�?//
// 为什�?ModelWhitelist 为空�?对所有模型生效）�?// codex 客户端的 service_tier=fast 是用户级开关，�?model 字段正交。即�?// 用户使用 gpt-4 + fast，priority 配额仍会被消耗。如果默认规则只�?// gpt-5.5*�?�?gpt-4 + fast 透传 priority 上游" 这条路径就会绕过策略�?// �?codex 真实语义对齐，默认对所有模型生效；管理员若需要只针对特定
// 模型，可�?admin UI 中显式配�?model_whitelist�?func DefaultOpenAIFastPolicySettings() *OpenAIFastPolicySettings {
