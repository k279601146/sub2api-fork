...
type UserEdges struct {
	...
	// IdeSessions holds the value of the ide_sessions edge.
	IdeSessions []*IDESession `json:"ide_sessions,omitempty"`
...
// IdeSessionsOrErr returns the IdeSessions value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) IdeSessionsOrErr() ([]*IDESession, error) {
	if e.loadedTypes[12] {
		return e.IdeSessions, nil
	}
	return nil, &NotLoadedError{edge: "ide_sessions"}
}
...
// QueryIdeSessions queries the "ide_sessions" edge of the User entity.
func (_m *User) QueryIdeSessions() *IDESessionQuery {
	return NewUserClient(_m.config).QueryIdeSessions(_m)
}
