...
type UserQuery struct {
	...
	withIdeSessions           *IDESessionQuery
...
// QueryIdeSessions chains the current query on the "ide_sessions" edge.
func (_q *UserQuery) QueryIdeSessions() *IDESessionQuery {
	query := (&IDESessionClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withIdeSessions:           _q.withIdeSessions.Clone(),
...
// WithIdeSessions tells the query-builder to eager-load the nodes that are connected to
// the "ide_sessions" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithIdeSessions(opts ...func(*IDESessionQuery)) *UserQuery {
	query := (&IDESessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withIdeSessions = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withIdeSessions; query != nil {
		if err := _q.loadIdeSessions(ctx, query, nodes,
			func(n *User) { n.Edges.IdeSessions = []*IDESession{} },
			func(n *User, e *IDESession) { n.Edges.IdeSessions = append(n.Edges.IdeSessions, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadIdeSessions(ctx context.Context, query *IDESessionQuery, nodes []*User, init func(*User), assign func(*User, *IDESession)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(idesession.FieldUserID)
	}
	query.Where(predicate.IDESession(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.IdeSessionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
